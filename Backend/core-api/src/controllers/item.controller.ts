...
export const addItem=asyncHandler(async(req,res)=>{
    const{name,category,quantity,expiryDate,isEstimatedExpiry,source}=req.body
    ...
    const newItem=await item.create({user:req.user._id,name,category,quantity,expiryDate,isEstimatedExpiry:isEstimatedExpiry||false,source,status:"AVAILABLE"})
...
export const editItem=asyncHandler(async(req,res)=>{
    const {name,category,quantity,expiryDate,isEstimatedExpiry}=req.body
    const id=req.params.id as string
    if(!mongoose.Types.ObjectId.isValid(id)){
        throw new ApiError(400,"Invalid item")
    }
    ...
    {
        $set:{name,category,quantity,expiryDate,isEstimatedExpiry}
    },{new:true})
    if(!foodItem){
        throw new ApiError(404,"Item not found")
    }
